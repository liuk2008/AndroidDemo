...
public class RetrofitDemo {
    ...
    private static FinanceRequest apiRequest = FinanceRequest.getInstance();

    public static void login() {
        RxHelper.subscribe(apiRequest.login("18909131172", "123qwe"), new Callback<UserInfo>() {
            @Override
    ...
    public static void checkPhone() {
        RxHelper.subscribe(apiRequest.checkPhone("18909131173"), new Callback<LinkedHashMap<String, Object>>() {
            @Override
...
//                                sendLoginSms(model);
    ...
    public static void sendLoginSms(GeeValidateInfo model) {
        RxHelper.subscribe(apiRequest.sendLoginSms("18909131189", model), new Callback<LinkedHashMap<String, Object>>() {
            @Override
            public void onSuccess(LinkedHashMap<String, Object> linkedHashMap) {
                Set<String> keys = linkedHashMap.keySet();
                for (String key : keys) {
    ...
    public static void monthBill() {
        RxHelper.subscribe(apiRequest.monthBill(), new Callback<MonthBillInfo>() {
            @Override
